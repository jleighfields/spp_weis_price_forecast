...
| `modeling.py` | Builds and fits the Darts models (TiDE / TSMixer / TFT), loads a saved ensemble (`load_ensemble_from_dir`), and builds the per-band coverage metrics the study scores calibration with (`coverage_metric`). |
| `targets.py` | The forecast-target set (`TARGETS` / `DEFAULT_TARGET`), including each target's training-data `clip_quantiles`. Dependency-light (no sklearn/darts) so darts-free callers can import it; re-exported by `parameters.py`. |
