...
# set 32 bit precision to speed up training on compatible gpus
torch.set_float32_matmul_precision('medium')

# filter out training warnings
import warnings
...
def build_fit_tsmixerx(
    ...
    if callbacks:
        model_params['pl_trainer_kwargs'] = {"callbacks": callbacks}
    ...
    # add model type and train time to model
...
def get_ci_err(
    ...
    for i, pred in enumerate(pred_series):
        ...
        ci_cover_err += [100.0 * np.abs(cover - 0.8)]
